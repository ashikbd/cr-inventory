            ...
            <div class="card-body">
              ...
              <form method="post" action="{{ route('sales.update',$sale) }}">
                  ...
                  <select required name="client_id" class="form-control select2">
                    ...
                    @if($clients)
                      @foreach($clients as $client)
                        <option value="{{ $client->id }}" @if($sale->client_id == $client->id) selected @endif>{{ $client->first_name }} {{ $client->last_name }} [Mobile: {{ $client->mobile }} | Email: {{ $client->email }}]</option>
                      @endforeach
                    @endif
                  </select>
                </div>
                <table class="table table-bordered" id="services">
                            ...
                            <td>
                                <select class="form-control select2 service" required style="width:100%" name="service[]">
                                    ...
                                    @foreach($services as $row)
                                      <option value="{{$row->id}}" @if($item->service_id == $row->id) selected @endif data-price="{{$row->purchase_price}}">{{$row->name}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td><input type="text" required name="unit_price[]" value="{{ $item->price + $item->discount }}" class="form-control unit_price"></td>
                            <td><input type="text"  name="discount[]" class="form-control discount" value="{{ $item->discount }}"></td>
                            <td><input type="text" readonly name="price[]" value="{{ $item->price }}" class="form-control price"></td>
                            <td>
                    ...
                    <tfoot>
                      <tr>
                        ...
                        <td></td>
                        <td><input type="text" readonly name="total_discount" value="{{ $sale->total_discount }}" class="form-control total_discount" value="0"></td>
                        <td><input type="text" readonly name="total_price" value="{{ $sale->total_price }}" class="form-control total_price" value="0"></td>
                        <td></td>
                      </tr>
                    </tfoot>
                </table>
                ...
                <div class="form-group">
                  ...
                  <textarea id="notes" class="form-control" name="notes" rows="4">{{ $sale->notes }}</textarea>
                </div>
